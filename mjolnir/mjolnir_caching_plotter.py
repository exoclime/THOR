"""caching class data version of mjolnir_plot_helpers"""
...
class caching_plotter:
    
    def __init__(self, args):
        self.args = args
        pview = args.pview
        self.ntsi = args.initial_file[0]  # initial file id number

        if args.last_file[0] == 'init':
            self.nts = ntsi
        else:
            self.nts = args.last_file[0]     # last file id number
            
        if self.ntsi > self.nts:
            self.nts = self.ntsi
                
        self.resultsf = args.file[0]
        if args.simulation_ID[0] == 'auto':
            ...
            self.simulation_ID = file0.split(sep='_')[2]
        else:
            self.simulation_ID = args.simulation_ID[0]

        if args.vcoord[0] == 'pressure':
            self.use_p = True
        elif args.vcoord[0] == 'height':
            self.use_p = False
            self.plog = False  # enforce this on height grid
        else:
            raise ValueError('%s not a valid vcoord. Valid options are "pressure" or "height"' % args.vcoord[0])

        
        self.pview = args.pview

        self.maketable = args.maketable
        self.plog = True
        ...
        self.input_data = self.outall.input

        ########
        # Grid #
        ########
        
        self.grid = self.outall.grid
    ...
    # Plots #
    #########
    def make_plot(self, pview_one, save=True, axis=None, override_args={}):
        openrg = self.openrg
        pview = [pview_one]
        grid =self.grid
        use_p = self.use_p

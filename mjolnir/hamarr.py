...
class grid:
    def __init__(self, resultsf, simID, rotation=False, theta_y=0, theta_z=0):
        ...
        self.pntloc = np.reshape(openh5['pntloc'][...], (self.point_num, 6))
        ...
        if 'curlz' in openh5.keys():
            self.curlz = np.reshape(
                openh5['curlz'][...], (self.point_num, 7, 3))
        openh5.close()
        self.nvi = self.nv+1
    ...
    def __init__(self, resultsf, simID, ntsi, nts, input, grid, stride=1):
        ...
        for t in np.arange(ntsi-1, nts, stride):
            ...
            if 'Etotal' in openh5.keys():
                ...
                print(
                    'Warning: conservation diagnostics not available in file %s' % fileh5)
            ...
            self.Rho[:, :, t-ntsi +
                     1] = np.reshape(Rhoi, (grid.point_num, grid.nv))
            self.Pressure[:, :, t-ntsi +
                          1] = np.reshape(Pressurei, (grid.point_num, grid.nv))
            self.Mh[0, :, :, t-ntsi +
                    1] = np.reshape(Mhi[::3], (grid.point_num, grid.nv))
            self.Mh[1, :, :, t-ntsi +
                    1] = np.reshape(Mhi[1::3], (grid.point_num, grid.nv))
            self.Mh[2, :, :, t-ntsi +
                    1] = np.reshape(Mhi[2::3], (grid.point_num, grid.nv))
            self.Wh[:, :, t-ntsi +
                    1] = np.reshape(Whi, (grid.point_num, grid.nvi))
            self.Rd[:, :, t-ntsi +
                    1] = np.reshape(Rdi, (grid.point_num, grid.nv))
            self.Cp[:, :, t-ntsi +
                    1] = np.reshape(Cpi, (grid.point_num, grid.nv))

            self.time[t-ntsi+1] = time
            self.nstep[t-ntsi+1] = nstep
            if 'Etotali' in locals():
                self.Etotal[:, :, t-ntsi +
                            1] = np.reshape(Etotali, (grid.point_num, grid.nv))
                self.Mass[:, :, t-ntsi +
                          1] = np.reshape(Massi, (grid.point_num, grid.nv))
                self.AngMomx[:, :, t-ntsi +
                             1] = np.reshape(AngMomxi, (grid.point_num, grid.nv))
                self.AngMomy[:, :, t-ntsi +
                             1] = np.reshape(AngMomyi, (grid.point_num, grid.nv))
                self.AngMomz[:, :, t-ntsi +
                             1] = np.reshape(AngMomzi, (grid.point_num, grid.nv))
            if 'Entropyi' in locals():
                self.Entropy[:, :, t-ntsi +
                             1] = np.reshape(Entropyi, (grid.point_num, grid.nv))

            if 'traceri' in locals():
                self.ch4[:, :, t-ntsi+1] = np.reshape(
                    traceri[::5], (grid.point_num, grid.nv))/self.Rho[:, :, t-ntsi+1]
                self.co[:, :, t-ntsi+1] = np.reshape(
                    traceri[1::5], (grid.point_num, grid.nv))/self.Rho[:, :, t-ntsi+1]
                self.h2o[:, :, t-ntsi+1] = np.reshape(
                    traceri[2::5], (grid.point_num, grid.nv))/self.Rho[:, :, t-ntsi+1]
                self.co2[:, :, t-ntsi+1] = np.reshape(
                    traceri[3::5], (grid.point_num, grid.nv))/self.Rho[:, :, t-ntsi+1]
                self.nh3[:, :, t-ntsi+1] = np.reshape(
                    traceri[4::5], (grid.point_num, grid.nv))/self.Rho[:, :, t-ntsi+1]

            if 'taui' in locals():
                self.tau_sw[:, :, t-ntsi +
                            1] = np.reshape(taui[::2], (grid.point_num, grid.nv))
                self.tau_lw[:, :, t-ntsi +
                            1] = np.reshape(taui[1::2], (grid.point_num, grid.nv))
                self.flw_dn[:, :, t-ntsi +
                            1] = np.reshape(fdni, (grid.point_num, grid.nvi))
                self.flw_up[:, :, t-ntsi +
                            1] = np.reshape(fupi, (grid.point_num, grid.nvi))
                self.fsw_dn[:, :, t-ntsi +
                            1] = np.reshape(sdni, (grid.point_num, grid.nvi))
        ...
        for t in np.arange(ntsi-1, nts):
            ...
            if t == ntsi-1:
                ...
                if RT:
                    ...
                    if surf:
                        self.Tsurface = np.zeros(
                            np.shape(Tsurfi)+(nts-ntsi+1,))
                if chem == 1:
            ...
            if pressure_vert == True:
                ...
                if t > ntsi-1:
                    if (Prei == self.Pressure[:, 0]).all() == False:
                        print(
                            'Warning: Pressure grid in file %d does not match file %d' % (t, ntsi))
            else:
...
class GetOutput:
    def __init__(self, resultsf, simID, ntsi, nts, stride=1, openrg=0, pressure_vert=True, rotation=False, theta_y=0, theta_z=0, pgrid_ref='auto'):
        ...
        self.grid = grid(resultsf, simID, rotation=rotation,
                         theta_y=theta_y, theta_z=theta_z)
        self.output = output(resultsf, simID, ntsi, nts,
                             self.input, self.grid, stride=stride)
        if openrg == 1:
...
def define_Pgrid(resultsf, simID, ntsi, nts, stride, overwrite=False):
    ...
    for i in np.arange(num_out):
        ...
        pressure_mean[:, :, i] = np.reshape(
            openh5['Pressure_mean'][...], (point_num, nv))
        openh5.close()

    pgrid = np.mean(np.mean(pressure_mean, axis=0), axis=1)
    if surf == 1:
        extrap_low = (Altitudeh[0]-Altitude[1])/(Altitude[0]-Altitude[1])
        Psurf = pressure_mean[:, 1, :]+extrap_low * \
            (pressure_mean[:, 0, :]-pressure_mean[:, 1, :])
...
def create_rg_map(resultsf, simID, rotation=False, theta_z=0, theta_y=0):
    outall = GetOutput(resultsf, simID, 0, 0, rotation=rotation,
                       theta_z=theta_z, theta_y=theta_y)
    input = outall.input
    grid = outall.grid
...
def vertical_regrid_field(source_array, nv, x, xnew):
    ...
    if (x_non_mono_check == 1).any():
        raise ValueError(
            'Pressure interpolation failed! Pressure not monotonically decreasing with height')
    if (xnew_non_mono_check[0] == 1):
        raise ValueError(
            'Pressure interpolation failed! Destination pgrid not monotonically decreasing with height')
    return ynew
...
def regrid(resultsf, simID, ntsi, nts, pgrid_ref='auto', overwrite=False, comp=4,
    ...
    if not os.path.exists(resultsf+'/regrid_map.npz'):
        ...
        create_rg_map(resultsf, simID, rotation=rotation,
                      theta_z=theta_z, theta_y=theta_y)
    ...
    outall = GetOutput(resultsf, simID, ntsi, ntsi,
                       rotation=rotation, theta_z=theta_z, theta_y=theta_y)
    input = outall.input
    grid = outall.grid
    ...
    if pgrid_ref == 'auto':
        try:
            files_found = spr.check_output(
                'ls '+resultsf+'/pgrid_*.txt', shell=True).split()
        except:
            raise IOError(
                'No pgrid file found in "%s/", please run "pgrid -i <first> -l <last> %s"' % (resultsf, resultsf))
        if len(files_found) > 1:
            raise IOError(
                'Multiple pgrid files found in "%s/", please specify with -pgrid flag' % resultsf)
        else:
        ...
        if os.path.exists(resultsf+'/'+pgrid_ref):
            ...
            raise IOError('pgrid file %s not found!' %
                          (resultsf+'/'+pgrid_ref))
        ...
        if input.core_benchmark[0] == 0:
            if input.RT:
                ...
                if input.surface:
                    surf = 1
                    extrap_low = (
                        grid.Altitudeh[0]-grid.Altitude[1])/(grid.Altitude[0]-grid.Altitude[1])
                    Psurf = output.Pressure[:, 1, :]+extrap_low * \
                        (output.Pressure[:, 0, :]-output.Pressure[:, 1, :])
            else:
                RT = 0
                surf = 0
    ...
    for t in np.arange(ntsi, nts+1):
        ...
        if os.path.exists(fileh5p) or os.path.exists(fileh5h):  # regrid files exist
            if overwrite == True:  # overwrite existing files
                ...
                print(fileh5p+'or '+fileh5h +
                      ' already present! Skipping time = %d' % t)
        ...
        if proceed == 1:
            ...
            outall = GetOutput(
                resultsf, simID, t, t, rotation=rotation, theta_z=theta_z, theta_y=theta_y)
            output = outall.output

            # interpolate in z to middle of layers using this
            interpz = (
                grid.Altitude-grid.Altitudeh[:-1])/(grid.Altitudeh[1:]-grid.Altitudeh[:-1])
            ...
            if RT == 1:
                source['flw_up'] = output.flw_up[:, :-1, 0] + \
                    (output.flw_up[:, 1:, 0] -
                     output.flw_up[:, :-1, 0])*interpz[None, :]
                source['flw_dn'] = output.flw_dn[:, :-1, 0] + \
                    (output.flw_dn[:, 1:, 0] -
                     output.flw_dn[:, :-1, 0])*interpz[None, :]
            ...
            source['V'] = (-source['Mh'][0]*np.sin(grid.lat[:, None])*np.cos(grid.lon[:, None])
                           - source['Mh'][1] *
                           np.sin(grid.lat[:, None])*np.sin(grid.lon[:, None])
            ...
            for key in interm.keys():
                if np.shape(interm[key]) == (d_lon[0], d_lon[1]):
                    ...
                    tmp = np.sum(weight3[:, :, None] *
                                 source[key][near3], axis=1)
                    interm[key][:, :] = tmp.reshape(
                        (d_lon[0], d_lon[1], grid.nv))

            # dealing with RV and PV
            # potential temp (only for constant Rd and Cp, currently)
            pt = interm['Temperature'] * \
                (interm['Pressure']/input.P_Ref)**(-interm['Rd']/interm['Cp'])
            dptdz = np.gradient(pt, grid.Altitude, axis=2)
            # these are not very accurate at the 'edges'
            dptdlat = np.gradient(pt, lat_range, axis=0) / \
                (input.A+grid.Altitude[None, None, :])
            dptdlon = np.gradient(pt, lon_range, axis=1)/(input.A +
                                                          grid.Altitude[None, None, :])/np.cos(lat_range[:, None, None])
            ...
            interm['PV'] = (curlVz*dptdz + curlVlat*dptdlat +
                            curlVlon*dptdlon)/interm['Rho']
            ...
            for key in dest.keys():
                if np.shape(interm[key]) == (d_lon[0], d_lon[1]):
                    ...
                    dest[key][:, :, :] = vertical_regrid_field(
                        interm[key][:, :, ::-1], grid.nv, x, xnew)[:, :, ::-1]
...
def KE_spect(input, grid, output, sigmaref, coord='icoh', lmax_adjust=0):
    ...
    if coord == 'icoh':
        ...
        if tsp == 1:
            for lev in np.arange(grid.nv):
                KE_coeffs[:, :, :, lev, 0], chiz = chairs.expand.SHExpandLSQ(
                    KE[:, lev, 0], grid.lat*180/np.pi, grid.lon*180/np.pi, lmax)
                KE_power[:, lev, 0] = chairs.spectralanalysis.spectrum(
                    KE_coeffs, unit='per_lm')
                ax.plot(waven, KE_power[:, lev, 0],
                        'k-', c=cmap(lev/grid.nv), lw=1)
        else:
            for t in np.arange(tsp):
                KE_coeffs[:, :, :, grid.nv-1, t], chiz = chairs.expand.SHExpandLSQ(
                    KE[:, grid.nv-1, t], grid.lat*180/np.pi, grid.lon*180/np.pi, lmax)
                KE_power[:, grid.nv-1,
                         t] = chairs.spectralanalysis.spectrum(KE_coeffs, unit='per_lm')
                ax.plot(waven, KE_power[:, grid.nv-1, t],
                        'k-', c=cmap(t/tsp), lw=1)
    ...
    ax.set_yscale('log')
    ax.set_xscale('log')
    ax.vlines(lmax_grid, ax.get_ylim()[0], ax.get_ylim()[
              1], zorder=1000, linestyle='--')
    ...
    plt.tight_layout()
    plt.savefig(input.resultsf+'/figures/KEspectrum_%i_%i_%s.pdf' %
                (output.ntsi, output.nts, coord))
    plt.close()
    ...
    plt.subplot(2, 1, 1)
    if coord == 'llp':
        plt.imshow(KE[:, :, 0, 0], origin='lower', extent=(
            0, 360, -90, 90), norm=norm, aspect='auto')
    if coord == 'icoh':
        plt.tricontourf(grid.lon*180/np.pi, grid.lat *
                        180/np.pi, KE[:, 0, 0], levels=30)
    ...
    plt.subplot(2, 1, 2)
    plt.imshow(np.real(KEcomp), origin='lower', extent=(
        0, 360, -90, 90), norm=norm, aspect='auto')
    ...
    plt.savefig(input.resultsf+'/figures/KEmap_lowest_%i_%s.pdf' %
                (output.ntsi, coord))
    plt.close()
...
def vertical_lat(input, grid, output, rg, sigmaref, z, slice=[0, 360], save=True, axis=False, csp=500, wind_vectors=False, use_p=True):
    ...
    if len(slice) == 2:
        ...
        if slice[1] < slice[0]:
            raise IOError(
                "'slice' values must be arranged (small, large) because I am too lazy to code it better")
        if slice[0] < 0:
        ...
        if tsp > 1:
            ...
            Zonallt = np.nanmean(z['value'][:, :, :, 0]
                                 [:, mask_ind[:, 0], :], axis=1)
            # Vlt = np.nanmean(rg.V[:,:,:,0],axis=1)
            # Wlt = np.nanmean(rg.W[:,:,:,0],axis=1)
            if wind_vectors == True:
                Vlt = np.nanmean(
                    rg.V[:, :, :, 0][:, mask_ind[:, 0], :], axis=1)
                Wlt = np.nanmean(
                    rg.W[:, :, :, 0][:, mask_ind[:, 0], :], axis=1)
        ...
        if slice[0] in lon:
            ...
            for t in tsp:
                for lev in np.arange(d_sig):
                    Zonall[:, 0, lev, tsp] = interp.griddata(
                        np.vstack([lon, lat]).T, z['value'][:, :, lev, tsp], (slice[0], lat))
                    if wind_vectors == True:
                        Vl[:, 0, lev, tsp] = interp.griddata(
                            np.vstack([lon, lat]).T, rg.V[:, :, lev, tsp], (slice[0], lat))
                        Wl[:, 0, lev, tsp] = interp.griddata(
                            np.vstack([lon, lat]).T, rg.W[:, :, lev, tsp], (slice[0], lat))
    ...
    if use_p:
        ...
        hrange = np.where(np.logical_and(rg.Altitude[:, 0] >= np.min(
            sigmaref), rg.Altitude[:, 0] <= np.max(sigmaref)))
    ...
    if isinstance(axis, axes.SubplotBase):
        C = axis.contourf(latp*180/np.pi, ycoord, zvals,
                          clevels, cmap=z['cmap'])
        ax = axis
    elif axis == False:
        plt.figure(figsize=(5, 4))
        C = plt.contourf(latp*180/np.pi, ycoord, zvals,
                         clevels, cmap=z['cmap'])
        ax = plt.gca()
    else:
        raise IOError(
            "'axis = {}' but {} is not an axes.SubplotBase instance".format(axis, axis))
    ...
    if isinstance(csp, list) or isinstance(csp, tuple):
        ...
        if csp == 'match':
            ...
            if use_p:
                levp = np.arange(np.ceil(np.nanmin(
                    Zonallt[:, prange[0]])/csp)*csp, np.floor(np.nanmax(Zonallt[:, prange[0]])/csp)*csp, csp)
            else:
                levp = np.arange(np.ceil(np.nanmin(
                    Zonallt[:, hrange[0]])/csp)*csp, np.floor(np.nanmax(Zonallt[:, hrange[0]])/csp)*csp, csp)

    c2 = ax.contour(latp*180/np.pi, ycoord, zvals,
                    levels=levp, colors='w', linewidths=1)
    ...
    if len(slice) == 2:
        ax.set_title('Time = %#.3f-%#.3f days, Lon = (%#.3f,%#.3f)' %
                     (output.time[0], output.time[-1], slice[0], slice[1]), fontsize=10)
    else:
        ax.set_title('Time = %#.3f-%#.3f days, Lon = (%#.3f,)' %
                     (output.time[0], output.time[-1], slice[0]), fontsize=10)
    ...
    if save == True:
        ...
        if len(slice) == 2:
            plt.savefig(input.resultsf+'/figures/%s_ver_i%d_l%d_lon%#.2f-%#.2f.pdf' %
                        (z['name'], output.ntsi, output.nts, slice[0], slice[1]))
        else:
            plt.savefig(input.resultsf+'/figures/%s_ver_i%d_l%d_lon%#.2f.pdf' %
                        (z['name'], output.ntsi, output.nts, slice[0]))
        plt.close()
    if z['mt'] == True:
        if len(slice) == 2:
            fname = '%s_ver_i%d_l%d_lon%#.2f-%#.2f.dat' % (
                z['name'], output.ntsi, output.nts, slice[0], slice[1])
        else:
            fname = '%s_ver_i%d_l%d_lon%#.2f.dat' % (
                z['name'], output.ntsi, output.nts, slice[0])
        if use_p:
            maketable(latp*180/np.pi, rg.Pressure[prange[0], 0]/1e5, Zonallt[:, prange[0]],
                      'Latitude(d)', 'Pressure(bar)', z['name'], input.resultsf, fname)
        else:
            maketable(latp*180/np.pi, rg.Altitude[hrange[0], 0], Zonallt[:, hrange[0]],
                      'Latitude(d)', 'Altitude(m)', z['name'], input.resultsf, fname)
...
def vertical_lon(input, grid, output, rg, sigmaref, z, slice=[0, 360], save=True, axis=False, csp=500, wind_vectors=False, use_p=True):
    ...
    if len(slice) == 2:
        ...
        if tsp > 1:
            Meridl = np.nanmean(z['value'][mask_ind[:, 0], :, :, :]*np.cos(
                lat[mask_ind[:, 0], 0]*np.pi/180)[:, None, None, None], axis=0)
            # Vl = np.nanmean(rg.V[:,:,:,:],axis=1)
            # Wl = np.nanmean(rg.W[:,:,:,:],axis=1)
            ...
            del Meridl
            if wind_vectors == True:
                Ul = np.nanmean(rg.U[mask_ind[:, 0], :, :, :]*np.cos(
                    lat[mask_ind[:, 0], 0]*np.pi/180)[:, None, None, None], axis=0)
                Wl = np.nanmean(rg.W[mask_ind[:, 0], :, :, :]*np.cos(
                    lat[mask_ind[:, 0], 0]*np.pi/180)[:, None, None, None], axis=0)
                Ult = np.nanmean(Ul[:, :, :], axis=2)
                Wlt = np.nanmean(Wl[:, :, :], axis=2)
                del Ul, Wl
        else:
            Meridlt = np.nanmean(z['value'][:, :, :, 0][mask_ind[:, 0], :, :]
                                 * np.cos(lat[mask_ind[:, 0], 0]*np.pi/180)[:, None, None], axis=0)
            # Ult = np.nanmean(rg.V[:,:,:,0],axis=1)
            # Wlt = np.nanmean(rg.W[:,:,:,0],axis=1)
            if wind_vectors == True:
                Ult = np.nanmean(rg.U[:, :, :, 0][mask_ind[:, 0], :, :]*np.cos(
                    lat[mask_ind[:, 0], 0]*np.pi/180)[:, None, None], axis=0)
                Wlt = np.nanmean(rg.W[:, :, :, 0][mask_ind[:, 0], :, :]*np.cos(
                    lat[mask_ind[:, 0], 0]*np.pi/180)[:, None, None], axis=0)
        ...
        if slice[0] in lat:
            ...
            for t in tsp:
                for lev in np.arange(d_sig):
                    Meridl[0, :, lev, tsp] = interp.griddata(
                        np.vstack([lon, lat]).T, z['value'][:, :, lev, tsp], (lon, slice[0]))
                    if wind_vectors == True:
                        Ul[0, :, lev, tsp] = interp.griddata(
                            np.vstack([lon, lat]).T, rg.U[:, :, lev, tsp], (lon, slice[0]))
                        Wl[0, :, lev, tsp] = interp.griddata(
                            np.vstack([lon, lat]).T, rg.W[:, :, lev, tsp], (lon, slice[0]))
    ...
    if isinstance(axis, axes.SubplotBase):
        C = axis.contourf(lonp*180/np.pi, ycoord, zvals,
                          clevels, cmap=z['cmap'])
        ax = axis
    elif axis == False:
        plt.figure(figsize=(5, 4))
        C = plt.contourf(lonp*180/np.pi, ycoord, zvals,
                         clevels, cmap=z['cmap'])
        ax = plt.gca()
    else:
        raise IOError(
    ...
    if isinstance(csp, list) or isinstance(csp, tuple):
        ...
        if csp == 'match':
            ...
            if use_p:
                levp = np.arange(np.ceil(np.nanmin(
                    Meridlt[:, prange[0]])/csp)*csp, np.floor(np.nanmax(Meridlt[:, prange[0]])/csp)*csp, csp)
            else:
                levp = np.arange(np.ceil(np.nanmin(
                    Meridlt[:, hrange[0]])/csp)*csp, np.floor(np.nanmax(Meridlt[:, hrange[0]])/csp)*csp, csp)

    c2 = ax.contour(lonp*180/np.pi, ycoord, zvals,
                    levels=levp, colors='w', linewidths=1)
    plt.clabel(c2, inline=False, fontsize=6, fmt='%d', use_clabeltext=True)
    for cc in C.collections:
        cc.set_edgecolor("face")  # fixes a stupid bug in matplotlib 2.0
    ...
    if use_p:
        ax.set_ylabel('Pressure (bar)')
        ax.plot(lonp*180/np.pi, np.zeros_like(lonp) +
                np.max(output.Pressure[:, grid.nv-1, :])/1e5, 'r--')
        ax.set_ylim(np.max(rg.Pressure[prange[0], 0])/1e5, np.min(Pref)/1e5)
    ...
    if len(slice) == 2:
        ax.set_title('Time = %#.3f-%#.3f days, Lat = (%#.3f,%#.3f)' %
                     (output.time[0], output.time[-1], slice[0], slice[1]), fontsize=10)
    else:
        ax.set_title('Time = %#.3f-%#.3f days, Lat = (%#.3f,)' %
                     (output.time[0], output.time[-1], slice[0]), fontsize=10)
    ...
    if save == True:
        # save the plot to file designated by z
        if len(slice) == 2:
            plt.savefig(input.resultsf+'/figures/%s_ver_i%d_l%d_lat%#.2f-%#.2f.pdf' %
                        (z['name'], output.ntsi, output.nts, slice[0], slice[1]))
        else:
            plt.savefig(input.resultsf+'/figures/%s_ver_i%d_l%d_lat%#.2f.pdf' %
                        (z['name'], output.ntsi, output.nts, slice[0]))
        plt.close()
    if z['mt'] == True:
        if len(slice) == 2:
            fname = '%s_ver_i%d_l%d_lat%#.2f-%#.2f.dat' % (
                z['name'], output.ntsi, output.nts, slice[0], slice[1])
        else:
            fname = '%s_ver_i%d_l%d_lat%#.2f.dat' % (
                z['name'], output.ntsi, output.nts, slice[0])
        if use_p:
            maketable(lonp*180/np.pi, rg.Pressure[prange[0], 0]/1e5, Meridlt[:, prange[0]],
                      'Longitude(d)', 'Pressure(bar)', z['name'], input.resultsf, fname)
        else:
...
def horizontal_lev(input, grid, output, rg, Plev, z, save=True, axis=False, wind_vectors=False, use_p=True):
    ...
    for t in np.arange(tsp):
        ...
        if len(np.shape(z['value'])) == 4:
            ...
            if use_p:
                title = 'Time = %#.3f-%#.3f days, Plev = %#.3f bar' % (
                    output.time[0], output.time[-1], Plev/1e5)
                fname = '%s_lev%#.3fmbar_i%d_l%d' % (
                    z['name'], Plev/100, output.ntsi, output.nts)
            else:
                title = 'Time = %#.3f-%#.3f days, lev = %#.3f km' % (
                    output.time[0], output.time[-1], Plev/1e3)
                fname = '%s_lev%#.3fkm_i%d_l%d' % (
                    z['name'], Plev/1000, output.ntsi, output.nts)
        elif len(np.shape(z['value'])) == 3:
            # 2D field, only one level available (e.g., insolation or surface temp)
            zlev[:, :, t] = z['value'][:, :, t]
            title = 'Time = %#.3f-%#.3f days' % (
    ...
    if wind_vectors == True:
        ...
        ax.quiverkey(q, X=0.85, Y=-0.1, U=np.max(np.sqrt(U**2+V**2)),
                     label='%#.2f m/s' % np.max(np.sqrt(U**2+V**2)), labelpos='E')
    ...
    if z['mt'] == True:
        ...
        if z['name'] == 'temperature-uv':
            ...
            zname = z['name']
        maketable(lonp, latp, zlevt.T, 'Longitude(d)',
                  'Latitude(d)', zname, input.resultsf, dname)
...
def streamf_moc_plot(input, grid, output, rg, sigmaref, save=True, axis=False, wind_vectors=False, mt=False, plog=True):
    ...
    arg = 2*np.pi*input.A * \
        np.cos(rg.lat[:, 0][:, None]*np.pi/180)/input.Gravit*Vavglt
    ...
    # Contour plot
    if isinstance(axis, axes.SubplotBase):
        C = axis.contourf(rg.lat[:, 0], rg.Pressure[prange[0], 0] /
                          1e5, sf[:, prange[0]].T, 40, cmap='viridis')
        ax = axis
    elif axis == False:
        plt.figure(figsize=(5, 4))
        C = plt.contourf(rg.lat[:, 0], rg.Pressure[prange[0], 0] /
                         1e5, sf[:, prange[0]].T, 40, cmap='viridis')
        ax = plt.gca()
    else:
        raise IOError(
    ...
    if wind_vectors == True:
        ...
        #preq = rg.Pressure[:,0][::spacing,::spacing].ravel()
        #latq = lati[::spacing,::spacing].ravel()
        latq, preq = np.meshgrid(
            rg.lat[::vspacing, 0], rg.Pressure[::wspacing, 0][prange[0]])
        del Vlt, Wlt
        plt.quiver(latq.ravel(), preq.ravel()/1e5, Vq, Wq, color='0.5')

    ax.invert_yaxis()
    c2 = ax.contour(rg.lat[:, 0], rg.Pressure[:, 0]/1e5,
                    sf.T, levels=[0.0], colors='w', linewidths=1)
    clb = plt.colorbar(C)
    ...
    ax.set_title('Time = %#.1f-%#.1f days, Lon = (0,360)' %
                 (output.time[0], output.time[-1]), fontsize=10)
    if not os.path.exists(input.resultsf+'/figures'):
        os.mkdir(input.resultsf+'/figures')
    plt.tight_layout()
    if save == True:
        plt.savefig(input.resultsf+'/figures/streamf_ver_i%d_l%d.pdf' %
                    (output.ntsi, output.nts))
        plt.close()
    if mt == True:
...
def profile(input, grid, output, z, stride=50):
    ...
    for column in np.arange(0, grid.point_num, stride):
        ...
        plt.plot(x[np.int(np.floor(grid.nv/2))],
                 P[np.int(np.floor(grid.nv/2))]/100000, 'r+', ms=5, alpha=0.5)
        plt.plot(x[np.int(np.floor(grid.nv*0.75))],
                 P[np.int(np.floor(grid.nv*0.75))]/100000, 'g+', ms=5, alpha=0.5)
    ...
    if not os.path.exists(input.resultsf+'/figures'):
        os.mkdir(input.resultsf+'/figures')
    plt.savefig(input.resultsf+'/figures/%sPprofile_i%d_l%d.pdf' %
                (z['name'], output.ntsi, output.nts))
    plt.close()
...
def CalcE_M_AM(input, grid, output, split):
    ...
    rint = ((input.A+grid.Altitudeh[1:])**3 -
            (input.A+grid.Altitudeh[:-1])**3)/3.0
    ...
    rx = r[None, :, None] * \
        np.cos(grid.lat[:, None, None])*np.cos(grid.lon[:, None, None])
    ry = r[None, :, None] * \
        np.cos(grid.lat[:, None, None])*np.sin(grid.lon[:, None, None])
    ...
    if split == False:
        ...
        for ii in np.arange(np.shape(output.Pressure)[2]):
            output.WeatherE[ii] = np.sum(
                output.Etotal[output.Pressure[:, :, ii] <= split][:, ii])
            output.DeepE[ii] = np.sum(
                output.Etotal[output.Pressure[:, :, ii] > split][:, ii])
            output.WeatherMass[ii] = np.sum(
                output.Mass[output.Pressure[:, :, ii] <= split][:, ii])
            output.DeepMass[ii] = np.sum(
                output.Mass[output.Pressure[:, :, ii] > split][:, ii])
            output.WeatherAMx[ii] = np.sum(
                output.AngMomx[output.Pressure[:, :, ii] <= split][:, ii])
            output.DeepAMx[ii] = np.sum(
                output.AngMomx[output.Pressure[:, :, ii] > split][:, ii])
            output.WeatherAMy[ii] = np.sum(
                output.AngMomy[output.Pressure[:, :, ii] <= split][:, ii])
            output.DeepAMy[ii] = np.sum(
                output.AngMomy[output.Pressure[:, :, ii] > split][:, ii])
            output.WeatherAMz[ii] = np.sum(
                output.AngMomz[output.Pressure[:, :, ii] <= split][:, ii])
            output.DeepAMz[ii] = np.sum(
                output.AngMomz[output.Pressure[:, :, ii] > split][:, ii])
...
def CalcEntropy(input, grid, output, split):
    ...
    if split == False:
        ...
        for ii in np.arange(np.shape(output.Pressure)[2]):
            output.WeatherEnt[ii] = np.sum(
                output.Entropy[output.Pressure[:, :, ii] <= split][:, ii])
            output.DeepEnt[ii] = np.sum(
                output.Entropy[output.Pressure[:, :, ii] > split][:, ii])
...
def conservation(input, grid, output, split):
    ...
    for ii in np.arange(len(plots)):
        ...
        fig.suptitle('Time = %#.3f - %#.3f days, split = %e bar' %
                     (output.time[0], output.time[-1], split/1e5))
        fig.subplots_adjust(wspace=0.25, left=0.07,
        ...
        if split == False:
            ...
            plt.plot(output.time, output.GlobalAMy,
                     'o', color='0.5', linestyle='--')
        else:
            if plots[ii] == 'weather':
                ...
                plt.plot(output.time, output.DeepAMy, 'o',
        ...
        plt.subplot(2, 3, 6)
        if split == False:
            plt.plot(output.time, np.sqrt(output.GlobalAMx**2 +
                                          output.GlobalAMy**2), 'ko', linestyle='--')
        else:
            if plots[ii] == 'weather':
                plt.plot(output.time, np.sqrt(output.WeatherAMx**2 +
                                              output.WeatherAMy**2), 'bo', linestyle='--')
            elif plots[ii] == 'deep':
                plt.plot(output.time, np.sqrt(output.DeepAMx**2 +
                                              output.DeepAMy**2), 'ro', linestyle='--')
        plt.xlabel('Time (days)')
        plt.ylabel(
            r'Horizontal angular momentum of atmosphere (kg m$^2$ s$^{-1}$)')

        if not os.path.exists(input.resultsf+'/figures'):
            os.mkdir(input.resultsf+'/figures')
        plt.savefig(input.resultsf+'/figures/conservation_s%e_i%d_l%d_%s.pdf' %
                    (split/100, output.ntsi, output.nts, plots[ii]))
        plt.close()
...
def SRindex(input, grid, output):
    ...
    U = (-output.Mh[0]*np.sin(grid.lon[:, None, None]) +
    ...
    mom = (input.A+grid.Altitude[None, :, None])*np.cos(grid.lat[:, None, None]) *\
          ((input.A+grid.Altitude[None, :, None]) *
           input.Omega*np.cos(grid.lat[:, None, None])+U)
    mom0 = (input.A+grid.Altitude[None, :, None])*np.cos(grid.lat[:, None, None]) *\
        ((input.A+grid.Altitude[None, :, None])*input.Omega *
         np.cos(grid.lat[:, None, None])+np.zeros(np.shape(U[:, :, :])))
    ...
    if not os.path.exists(input.resultsf+'/figures'):
        os.mkdir(input.resultsf+'/figures')
    plt.savefig(input.resultsf+'/figures/SRindex_i%d_l%d.pdf' %
                (output.ntsi, output.nts))
    plt.close()
...
def Get_Prange(input, grid, rg, args, xtype='lat', use_p=True):
    ...
    if use_p:
        ...
        if np.max(input.P_Ref)/np.float(args.vertical_top[0]) > 1000:
            sigmaref = np.logspace(np.log10(np.max(rg.Pressure)), np.log10(
                np.float(args.vertical_top[0])*100), 20)/input.P_Ref
        else:
            sigmaref = np.linspace(np.max(rg.Pressure), np.float(
                args.vertical_top[0])*100, 20)/input.P_Ref

    else:
        if (args.vertical_top[0] == 'default'):
            sigmaref = grid.Altitude
        else:
            sigmaref = grid.Altitude[np.where(grid.Altitude <= np.float(
                args.vertical_top[0])*input.Top_altitude)[0]]
    return sigmaref

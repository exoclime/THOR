...
__global__ void Diffusion_Op (double * diffmh_d     , 
    ...
    if (laststep) a_s[ir] = diff_d[id * nv * 6 + lev * 6 + var];
        ...
        if (var == 0) a_s[ir] = Rho_d[id * nv + lev];
        if (var == 1) a_s[ir] = Mh_d[id * nv * 3 + lev * 3 + 0];
        if (var == 2) a_s[ir] = Mh_d[id * nv * 3 + lev * 3 + 1];
        if (var == 3) a_s[ir] = Mh_d[id * nv * 3 + lev * 3 + 2];
        if (var == 4) a_s[ir] = W_d[id * nv + lev];
        if (var == 5) a_s[ir] = temperature_d[id * nv + lev];
    }
    ...
    if (x == 0) {
        ...
        if (laststep) a_s[ir2]   = diff_d[ig * nv * 6 + lev * 6 + var];
            ...
            if (var == 0) a_s[ir2] = Rho_d[ig * nv + lev];
            if (var == 1) a_s[ir2] = Mh_d[ig * nv * 3 + lev * 3 + 0];
            if (var == 2) a_s[ir2] = Mh_d[ig * nv * 3 + lev * 3 + 1];
            if (var == 3) a_s[ir2] = Mh_d[ig * nv * 3 + lev * 3 + 2];
            if (var == 4) a_s[ir2] = W_d[ig * nv + lev];
            if (var == 5) a_s[ir2] = temperature_d[ig * nv + lev];
        }
    ...
    if (laststep){
        if (var < 5) sdiff = -K_d[lev]     ;
        else         sdiff = -K_d[lev] * Rd;
    }
    ...
    if (laststep){
        ...
        if (var == 3){
            ...
            dmhz = lap;
            dmhr = funcx * diffmh_d[id * nv * 3 + lev * 3 + 0] + funcy * diffmh_d[id * nv * 3 + lev * 3 + 1] + funcz * dmhz;
...
/////////////////////////////////////////
    ...
    if (laststep){
        ...
        if (var == 1){
            ...
            for (int i = 1; i < 6; i++) a_p[i] = Mh_d[local_p[i - 1] * 3 * nv + lev * 3 + 0] / Rho_p[i];
        }
        if (var == 2){
            a_p[0] = Mh_d[id * 3 * nv + lev * 3 + 1] / Rho_p[0];
            for (int i = 1; i < 6; i++) a_p[i] = Mh_d[local_p[i - 1] * 3 * nv + lev * 3 + 1] / Rho_p[i];
        }
        if (var == 3){
            a_p[0] = Mh_d[id * 3 * nv + lev * 3 + 2] / Rho_p[0];
            for (int i = 1; i < 6; i++) a_p[i] = Mh_d[local_p[i - 1] * 3 * nv + lev * 3 + 2] / Rho_p[i];
        }
        if (var == 4) {

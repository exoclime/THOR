...
void print_def(data_def& d, int i) {
    std::string coord = "";
    if (d.fun != nullptr) coord = d.fun(i);
...
int main(int argc, char* argv[]) {
    ...
    std::vector<data_def> data_definitions = {
        {std::string("array1d"),
         kad.array1d,
         kad.array1d_size,
         std::bind(&knowsaboutdata::index_1d, &kad, _1)},
        {std::string("array2d"),
         kad.array2d,
         kad.array2d_size_x * kad.array2d_size_y,
         std::bind(&knowsaboutdata::index_2d, &kad, _1)},
    ...
    for (auto& def : data_definitions) { print_def(def, 7); }

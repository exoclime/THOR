    ...
    for (int rk = 0; rk < 3; rk++) {
        ...
        if (increased_damping_for_100_days == 1) {
            ...
            if (current_step < 100) {
                ...
                printf(
                    "changed Diffc in the first 100 time steps by a factor of %e for PARMENTIER \n",
                    Diffc_intitial_factor);
                printf("changed DivDampc in the first 100 time steps by a factor of %e for "
                       "PARMENTIER \n",
                       DivDampc_intitial_factor);
                printf("changed Diffc_v in the first 100 time steps by a factor of %e for "
                       "PARMENTIER \n",
                ...
                for (int lev = 0; lev < nv; lev++) {
                    ...
                    Kdh4_h[lev] = (Diffc_intitial_factor * sim.Diffc)
                                  * pow(dbar, 1.0 * sim.HyDiffOrder)
                                  / timestep; // * Altitude_h[lev]/sim.Top_altitude;
                    Kdhz_h[lev] = (DivDampc_intitial_factor * sim.DivDampc) * pow(dbar, 4.)
                                  / timestep; // * Altitude_h[lev]/sim.Top_altitude;
                    if (sim.DiffSponge) {
                ...
                for (int lev = 0; lev < nv; lev++) {
                    ...
                    Kdv6_h[lev] = Diffc_v_intitial_factor * sim.Diffc_v

...
void device_RK_array_manager::allocate_device_array() {
    cudaMemcpyToSymbol(
        dynamical_core_phy_modules_arrays, data.data(), data.size() * sizeof(device_RK_array));
    {
    ...
    for (auto& d : data)
        log::printf(
            "%d %p %p %p\n", d.dimensions, (void*)d.array_d, (void*)d.arrayk_d, (void*)d.arrayi_d);
#endif // __DEBUG


    // maybe this needs a pointer ?
    cudaMemcpyToSymbol(num_dynamical_arrays, &datasize, sizeof(int));
    {
        cudaError_t err = cudaGetLastError();
